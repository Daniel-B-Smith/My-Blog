...
static PyMethodDef classify_bad_methods[] =
  {
    {"classify", classify_bad_classify, METH_VARARGS, classify_docstring},
    {NULL, NULL, 0, NULL}
  };

PyMODINIT_FUNC initclassify_bad(void)
{
  PyObject *m = Py_InitModule3("classify_bad", classify_bad_methods, 
			       module_docstring);
  if (m==NULL) 
    return;
